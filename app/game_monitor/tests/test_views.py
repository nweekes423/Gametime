...
class ViewsTests(TestCase):
    ...
    def test_mock_nba_api_with_file(self):
        """Test mock_nba_api with valid file."""
        game_data = {
            "scoreboard": {
                "games": [
                    {
        ...
        with patch("builtins.open", mock_open(read_data=json.dumps(game_data))):
            response = self.client.get("/game-monitor/mock-api/")
            self.assertEqual(response.status_code, 200)
            
            data = response.json()
            self.assertIn("close_games", data)
            self.assertEqual(len(data["close_games"]), 1)

    def test_mock_nba_api_file_not_found(self):
        """Test mock_nba_api file not found."""
    ...
    def test_mock_nba_api_invalid_json(self):
        """Test mock_nba_api invalid JSON."""
    ...
    def test_mock_nba_api_invalid_structure(self):
        """Test mock_nba_api invalid structure."""
        with patch("builtins.open", mock_open(read_data='{"invalid": "structure"}')):
            response = self.client.get("/game-monitor/mock-api/")
            self.assertEqual(response.status_code, 500)
            data = response.json()
            self.assertEqual(data["error"], "Invalid scoreboard structure")

    def test_test_cache_view(self):
        """Test test_cache_view."""
        cache.clear()
        ...
        self.assertIn("fetch_source", data)
